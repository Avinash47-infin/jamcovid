 ...
 **/
 ...
 var formValidation = {
 		...
 		for(i in request) {
 			...
 			if(vr_key in validator) {
 				...
 				for(attr in conditions) {
 					...
 					var existing_error = (vr_key in response.errorMessages) ? response.errorMessages[vr_key] : "";
 					var existing_errorMsg = contain_array ? ( (existing_error.hasOwnProperty(inp_index)) ? existing_error[inp_index] : "" ) : existing_error;
 					if(existing_errorMsg.length == 0) {
 						switch(attr) {
 							...
 							if(threshold && input_value.length) {
 								if(isNaN(parseFloat(input_value)) || !$.isNumeric(input_value)) {
 									error 		   = condition['message'];
 									response.valid = false;
 								}
 							}
							...
							case "dob":
							if(input_value.length > 0) {
								var currentDate = moment().format(threshold);
								var selectedDate= moment(input_value, threshold).format(threshold);
								if(moment(selectedDate, threshold).isSameOrAfter(moment(currentDate, threshold))) {
									error 		   = condition['message'];
									response.valid = false;
								}
							}
						...
						if(contain_array) {
							...
							if(response.errorMessages[vr_key]) {
								var existingInfo = response.errorMessages[vr_key];
	...
	renderFormErrorMessages: function(form_id, errorMessages) {
		...
		for(name in errorMessages) {
			...
			if($.isArray(message) || typeof message === 'object') {
				for(m in message) {
					...
					$('#'+form_id+' [name="'+input_name+'"]').next().html(message[m]);
					if(message[m].length > 0) {
						$('#'+form_id+' [name="'+input_name+'"]').addClass('error_border');
						if(initialInputName == '') {
				...
				$('#'+form_id+' [name="'+name+'"]').next().html(message);
				if(message.length > 0) {
					$('#'+form_id+' [name="'+name+'"]').addClass('error_border');
					if(initialInputName == '') {
		...
		if(initialInputName != "") {
			var attrType = $('#'+form_id+' [name="'+initialInputName+'"]').prop('type');
			if(attrType == "hidden") {
				var focustInputId = $('#'+form_id+' [name="'+initialInputName+'"]').attr('focus-input-id');
				$('#'+form_id+' #'+focustInputId).focus();
			} else {
	...
	clearFormInputs: function(form_id, formInputs) {
		for(i in formInputs) {
			...
			if($('#'+form_id+' [name="'+name+'"]').next().hasClass('validation_error')) {
				$('#'+form_id+' [name="'+name+'"]').removeClass('error_border');
				$('#'+form_id+' [name="'+name+'"]').next().html('');
			}
		}
	},
...
//$(document).on('click', '.app_radio_validate', function() {
	$(document).on('change', 'input.app_radio_validate', function (event) {
		var name = $(this).attr('name');
		var checked = $(this).prop("checked") ? 1 : "";
		$('.'+name+'_radio').val(checked);
		if($('.'+name+'_radio').next().hasClass('validation_error')) {
			$('.'+name+'_radio').next().html('');
		}
	});